      ...
      <button type="button" onClick={() => setOpen((o) => !o)} className="w-full text-left p-5 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            <span className="text-[11px] font-bold uppercase tracking-[0.05em] px-2.5 py-1 rounded-full" style={{ backgroundColor: hexA(accent, 0.14), color: tagColor }}>
              {tag}
            </span>
            <p className="text-[12px] font-semibold uppercase tracking-[0.05em] text-[#A0AEC0]">{offering.workstream}</p>
          </div>
        ...
        <span className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center mt-0.5" style={{ backgroundColor: hexA(accent, open ? 0.18 : 0.1) }}>
          <ChevronDown size={16} style={{ color: accent === SECONDARY ? '#4A5568' : accent, transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s ease' }} />
        </span>
      </button>
          ...
          <ul className="space-y-2">
            {offering.valueDelivered.map((v, i) => (
              <li key={i} className="flex items-start gap-2 text-[13px] text-[#2D3748] leading-[1.55]">
                <span className="shrink-0 mt-[7px] w-1.5 h-1.5 rounded-full" style={{ backgroundColor: accent === SECONDARY ? '#A0AEC0' : accent }} />
                <span>{v}</span>
              </li>
            ))}
          </ul>
  ...
  return (
    <div className="rounded-xl p-5" style={{ backgroundColor: '#F7FAFC', border: '1px solid #E2E8F0' }}>
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <span className="text-[11.5px] font-semibold px-3 py-1 rounded-full" style={{ backgroundColor: '#EAF0F8', color: DARK, border: `1px solid ${PALE_BORDER}` }}>
          {rec.quadrantLabel}
        </span>
      </div>
